...
def test_login():
    data = {
        "username": "marium@gmail.com",
        "password": "Marium#5598"
    }
    response = client.post("/login", data=data)
    assert response.status_code == 200
    assert "access_token" in response.json()

# def test_show_appointment():
#     response = client.get("/showappointments/1")
#     assert response.status_code == 200
...
# def test_login():
    ...
    # assert response.status_code == 200
    # response = client.post("/login", json={"username": "charan@gmail.com", "password": "Charan#5598"})
